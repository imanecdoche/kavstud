      ...
      snapshot.forEach((docSnap) => {
        ...
        if (isTeacher) {
          ...
          notifMap.set(notifId, {
            ...
            message: `${sub.studentName || 'Seorang siswa'} baru saja mengumpulkan jawaban untuk tugas "${sub.assignmentTitle || 'Tugas'}". ${sub.score !== null && sub.score !== undefined ? `Nilai: ${sub.score}/100` : 'Periksa & berikan nilai.'}`,
            date: dtSubmitted.date,
            time: dtSubmitted.time,
            timestamp: dtSubmitted.timestamp,
            isRead: readNotifIds.has(notifId),
            actionUrl: '/assignments',
            actionLabel: 'Periksa & Nilai',
          ...
          if (sub.studentId === user.uid) {
            if (sub.status === 'graded') {
              ...
              notifMap.set(notifId, {
                ...
                message: `Nilai kamu untuk tugas "${sub.assignmentTitle}": ${sub.score ?? 0}/100.${sub.feedback ? ` Catatan Guru: "${sub.feedback}"` : ''}`,
                date: dtSubmitted.date,
                time: dtSubmitted.time,
                timestamp: dtSubmitted.timestamp,
                isRead: readNotifIds.has(notifId),
