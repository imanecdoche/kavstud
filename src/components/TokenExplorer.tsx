...
export default function TokenExplorer() {
  ...
  const colors: ColorToken[] = [
    ...
    { name: 'Secondary Slate', variable: 'text-gray-600', hex: '#4B5563', description: 'Teks deskripsi kedua, icon pendukung, border tidak aktif.', category: 'secondary' },
    ...
    { name: 'White Background', variable: 'bg-white', hex: '#FFFFFF', description: 'Kanvas utama dan halaman.', category: 'neutral' },
    { name: 'Surface Gray', variable: 'bg-gray-50', hex: '#F9FAFB', description: 'Latar belakang section, table head, card inner body.', category: 'neutral' },
    { name: 'Border Light', variable: 'border-gray-100', hex: '#F3F4F6', description: 'Garis pembatas panel, grid halus, separator.', category: 'neutral' },
    { name: 'Text Primary', variable: 'text-gray-900', hex: '#111827', description: 'Typography utama, judul, text dominan.', category: 'neutral' },
  ];
  ...
  const typography: TypographyToken[] = [
    ...
    { name: 'Small & Caption', specs: 'Inter, Medium, text-xs, text-gray-500', usage: 'Meta info, tanggal release, status indikator kecil.', example: 'Modified: 20 hours ago' },
  ...
  const shadows: ShadowToken[] = [
    { name: 'Small', value: 'shadow-xs', className: 'shadow-xs border border-gray-100/50', description: 'Subtle border shadow untuk input fields & card flat.' },
    { name: 'Medium', value: 'shadow-sm', className: 'shadow-sm border border-gray-100/70', description: 'Hover states untuk card, dropdown menu overlay.' },
    { name: 'Large', value: 'shadow-md', className: 'shadow-md border border-gray-100', description: 'Popup dialogs, Floating Action Buttons (FAB), Toast notification.' },
  ];
    ...
    <div className="space-y-12 max-w-5xl" id="token-explorer">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Dua aksen utama: Indigo premium dan Charcoal neutral gray.</p>
          </div>
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {filteredColors.map((color) => (
            <div 
              key={color.name} 
              className="border border-gray-100 bg-white rounded-xl p-4 shadow-2xs flex flex-col justify-between hover:border-gray-200 transition-all group"
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button 
                    ...
                    className="p-1 text-gray-400 hover:text-indigo-600 rounded-md hover:bg-gray-50 transition-colors"
                    title="Copy hex code"
                  >
                ...
                />
                <p className="text-[11px] text-gray-500 leading-normal">{color.description}</p>
              </div>

              <div className="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between text-[10px] font-mono text-gray-400">
                <span>{color.hex}</span>
                <span className="text-gray-500 font-medium group-hover:text-indigo-600 transition-colors">{color.variable}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Sistem font sans-serif Inter premium didukung oleh headline Space Grotesk yang modern.</p>
        </div>

        <div className="border border-gray-100 bg-white rounded-2xl overflow-hidden divide-y divide-gray-50 shadow-2xs">
          {typography.map((t) => (
            <div key={t.name} className="p-5 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between hover:bg-gray-50/50 transition-colors">
              <div className="space-y-1 max-w-xs">
                <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{t.name}</span>
                <p className="text-xs font-mono text-gray-500">{t.specs}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Sudut membulat yang konsisten dari elemen micro ke panel besar.</p>
          </div>

          <div className="space-y-3 bg-white border border-gray-100 rounded-2xl p-5 shadow-2xs">
            {radii.map((r) => (
              <div key={r.name} className="flex items-center gap-4 justify-between text-xs py-2 border-b border-gray-50 last:border-0 last:pb-0">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[11px] text-gray-500">{r.value}</span>
                  <div className={`w-12 h-12 bg-indigo-50 border-2 border-indigo-200/50 flex items-center justify-center text-indigo-600 font-bold ${r.className}`}>
                    Aa
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Memberikan kesan kedalaman layout 3D yang elegan dan minimal.</p>
          </div>
          ...
          <div className="space-y-3 bg-white border border-gray-100 rounded-2xl p-5 shadow-2xs">
            {shadows.map((s) => (
              <div key={s.name} className="flex items-center gap-4 justify-between text-xs py-2 border-b border-gray-50 last:border-0 last:pb-0">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[11px] text-gray-500">{s.value}</span>
                  <div className={`w-14 h-10 bg-white rounded-xl ${s.className}`} />
                </div>
              </div>
            ))}
          </div>
